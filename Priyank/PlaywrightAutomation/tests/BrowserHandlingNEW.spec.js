...
test("Browser handling - 1st way", async ({ page }) => {
  ...
  await page
    .getByPlaceholder("Search Amazon.in", { exact: true })
    .fill("Iphone16");
  await page.getByRole("button", { name: "Go", exact: true }).click();

  const phone = page.locator(
    "(//h2[@class='a-size-medium a-spacing-none a-color-base a-text-normal'])[1]",
  );
...
test("Browser handling - 2nd way", async ({ browser }) => {
  ...
  await page
    .locator(
      "(//h2[@class='a-size-medium a-spacing-none a-color-base a-text-normal'])[2]",
    )
    .click();
